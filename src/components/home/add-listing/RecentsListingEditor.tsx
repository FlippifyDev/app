...
const RecentsListingEditor: React.FC<Props> = ({ marketItem, cacheKey }) => {
    const user = useUser();
    ...
    async function handleAddItem() {
        ...
        if (!localListing.dateListed) {
            localListing.dateListed = formatDateToISO(new Date())
        }

        if (!localListing.purchase) {
            localListing.purchase = {};
            localListing.purchase.date = formatDateToISO(new Date());
        }
        ...
        if (error) {
            ...
            const cache = `${inventoryCacheKey}-${user?.id}`;
            await addCacheData(cache, { [item.itemId as string]: item });
            setShowSuccessModal(true);
            setMessage(message);
        }
...
export default RecentsListingEditor;
